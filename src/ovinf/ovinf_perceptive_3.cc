#include "ovinf/ovinf_perceptive_3.h"

namespace ovinf {

PerceptivePolicy3::~PerceptivePolicy3() {
  exiting_.store(true);
...
PerceptivePolicy3::PerceptivePolicy3(const YAML::Node &config)
    : BasePolicy(config) {
  // Read config
  ...
  worker_thread_ = std::thread(&PerceptivePolicy3::WorkerThread, this);
}

bool PerceptivePolicy3::WarmUp(RobotObservation<float> const &obs_pack) {
...
std::optional<PerceptivePolicy3::VectorT> PerceptivePolicy3::GetResult(
...
void PerceptivePolicy3::PrintInfo() {
...
void PerceptivePolicy3::WorkerThread() {
  if (realtime_) {
...
void PerceptivePolicy3::CreateLog(YAML::Node const &config) {
...
void PerceptivePolicy3::WriteLog(RobotObservation<float> const &obs_pack) {
